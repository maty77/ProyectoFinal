...
<?php require_once 'inc/barralateral.inc.php';  ?>
	...
	<section class="full-width pageContent">
      ...
      <?php } ?>
		...
		<div class="full-width divider-menu-h"></div>
          ...
          <form method="GET" action="compra.php" class="mdl-grid">
    <div class="mdl-cell mdl-cell--4-col">
        <label for="proveedor">Proveedor:</label>
        <select name="proveedor" id="proveedor" class="mdl-textfield__input">
            <option value="">Todos</option>
            <?php 
            $query = "SELECT id_proveedor, proveedor FROM proveedor";
            $result = mysqli_query($MiConexion, $query);
            while ($row = mysqli_fetch_assoc($result)) {
                $selected = (isset($_GET['proveedor']) && $_GET['proveedor'] == $row['id_proveedor']) ? 'selected' : '';
                echo "<option value='{$row['id_proveedor']}' $selected>{$row['proveedor']}</option>";
            }
            ?>
        </select>
    </div>
    ...
    <div class="mdl-cell mdl-cell--4-col">
        ...
        <select name="pago" id="pago" class="mdl-textfield__input">
            <option value="">Todas</option>
            <?php 
            $query = "SELECT id_pago, pago FROM tipopago";
            $result = mysqli_query($MiConexion, $query);
            while ($row = mysqli_fetch_assoc($result)) {
                $selected = (isset($_GET['pago']) && $_GET['pago'] == $row['id_pago']) ? 'selected' : '';
                echo "<option value='{$row['id_pago']}' $selected>{$row['pago']}</option>";
            }
            ?>
        </select>
    </div>
    ...
    <div class="mdl-cell mdl-cell--4-col">
        ...
        <input type="date" name="fecha_inicio" id="fecha_inicio" class="mdl-textfield__input" value="<?php echo isset($_GET['fecha_inicio']) ? $_GET['fecha_inicio'] : ''; ?>">
    </div>

    <div class="mdl-cell mdl-cell--4-col">
        <label for="fecha_fin">Hasta:</label>
        <input type="date" name="fecha_fin" id="fecha_fin" class="mdl-textfield__input" value="<?php echo isset($_GET['fecha_fin']) ? $_GET['fecha_fin'] : ''; ?>">
    </div>

    <div class="mdl-cell mdl-cell--12-col text-center">
        <button type="submit" class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored">Filtrar</button>
    </div>
</form>

<form action="Reportes/reporte_gral_compras.php" method="GET" target="_blank" class="mdl-grid">
    <input type="hidden" name="proveedor" value="<?php echo isset($_GET['proveedor']) ? $_GET['proveedor'] : ''; ?>">
    <input type="hidden" name="pago" value="<?php echo isset($_GET['pago']) ? $_GET['pago'] : ''; ?>">
    <input type="hidden" name="fecha_desde" value="<?php echo isset($_GET['fecha_inicio']) ? $_GET['fecha_inicio'] : ''; ?>">
    <input type="hidden" name="fecha_hasta" value="<?php echo isset($_GET['fecha_fin']) ? $_GET['fecha_fin'] : ''; ?>">

    <div class="mdl-cell mdl-cell--12-col text-center">
        <button type="submit" class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored">Generar Reporte</button>
    </div>
</form>
